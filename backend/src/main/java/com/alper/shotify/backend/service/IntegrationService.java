...
public class IntegrationService {
    ...
    public RecommendedSongsListResponseDTO analyzePhoto (String photoPath){
        ...
        RecommendedSongsListResponseDTO recommendedSongsResponse = webClient.post()
                .uri(songRecommendationUrl)
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(Collections.singletonMap("keywords", detectionResponse.getKeywords()))
                .retrieve()
                .bodyToMono(RecommendedSongsListResponseDTO.class)
                .block();

        if (recommendedSongsResponse == null || recommendedSongsResponse.getRecommendedSongs().isEmpty()) {
